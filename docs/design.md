...
| データソース | JSON でパッケージにバンドル（外部 DB 不使用） |
| 動作環境 | ローカル完結 |
...
ai-rotom/
├── packages/
│   ├── shared/         # 共有する型定義・ドメインロジック
│   └── mcp-server/     # MCP サーバー実装 + データ
├── docs/
│   └── design.md       # 本ドキュメント
...
|---|---|---|---|---|
| `packages/shared` | `@ai-rotom/shared` | 共有する型定義・ドメインロジック（Domain 層 + Application 層） | しない（内部用） | なし |
| `packages/mcp-server` | `ai-rotom` | MCP サーバー実装 + Infrastructure 層 + MVP 時のデータ内包 | する（`npx ai-rotom`） | `@ai-rotom/shared` |
...
| 原則 | 適用内容 |
|---|---|
| 依存性逆転の原則 | shared パッケージ内でドメイン層が他のレイヤーに依存しない |
| 単一責任の原則 | 各パッケージ・各レイヤーが明確に1つの責務を持つ |
| YAGNI | 現時点で不要な抽象化は行わない。ただし Repository の interface/impl 分離はテスタビリティのために導入する |

### 2.3 shared パッケージと各サーバーパッケージのレイヤー構成

shared パッケージに Domain 層と Application 層を配置し、Infrastructure 層は各サーバーパッケージ（mcp-server, 将来の api-server）に配置する。
...
└─────────────────────────────────────────────────┘
                       ▲
┌─────────────────────────────────────────────────┐
│  各サーバーパッケージ（mcp-server / api-server）   │
│  ┌─────────────────────────────────────┐         │
│  │        Infrastructure 層            │  ← Repository 実装（データアクセス）
│  └─────────────────────────────────────┘         │
└─────────────────────────────────────────────────┘
```
...
| レイヤー | 配置先パッケージ | 責務 | 配置するもの |
|---|---|---|---|
| Domain | shared | ビジネスルールの表現 | Entity, ValueObject, DomainService, Repository interface, ドメインの型定義 |
| Application | shared | ユースケースの実行 | UseCase（ダメージ計算・選出アドバイス・パーティ構築支援）、DTO |
| Infrastructure | mcp-server（MVP） | データへのアクセス | Repository 実装（JSON データからの取得・変換） |
| Infrastructure | api-server（将来） | データへのアクセス | Repository 実装（DB からの取得・変換） |
...
│   │   │   │   ├── stat-calculator.ts
│   │   │   │   └── speed-comparator.ts
│   │   │   └── repository/            # Repository interface のみ
...
└── mcp-server/
    ...
    ├── scripts/
    │   ├── fetch-base-data.ts
    │   ├── apply-champions-overrides.ts
    │   ├── validate-data.ts
    │   └── overrides/
    │       ├── pokemon-overrides.json
    │       ├── moves-overrides.json
    │       └── abilities-overrides.json
    └── tsconfig.json
```
...
LLM (Claude 等)
  │  MCP プロトコル
  ▼
mcp-server/tools  ─── ツール定義・入力バリデーション・レスポンス整形
  │  UseCase 呼び出し
  ▼
shared/application  ─── ユースケースの実行・オーケストレーション
  │  Domain モデル操作
  ▼
shared/domain  ─── ビジネスロジックの実行（ダメージ計算等）
  ▲
  │  Repository interface
mcp-server/infrastructure  ─── データ取得・ドメインモデルへの変換
  │
  ▼
mcp-server/data  ─── 静的データの提供（JSON）
```

### 2.6 拡張ポイント

将来的に API サーバー / Web UI を追加する場合、shared パッケージはそのまま再利用できる。

**MVP 構成:**

```
mcp-server（データ内包） ──depends──→  shared
```
...
新しいサーバーパッケージは shared の Application 層（UseCase）を利用し、自身の Infrastructure 層に Repository 実装を持つ。ドメインロジックの再実装は不要である。
...
以下のドメインモデルはすべて shared パッケージ（`packages/shared/src/`）内に配置する。
...
ダメージ計算の結果を表すドメイン層の ValueObject。
...
| moves | Move[] | 技構成（最大4つ） |
| item | Item \| null | 持ち物（未設定の場合は null） |
...
計算式（HP）：
```
HP実数値 = (種族値 × 2 + 個体値 + 能力ポイント × 2) × レベル / 100 + レベル + 10
```

計算式（HP 以外）：
```
実数値 = ((種族値 × 2 + 個体値 + 能力ポイント × 2) × レベル / 100 + 5) × 性格補正
```

※ レベル 50 では +1 能力ポイント = +1 実数値（性格補正前）となる。
...
mcp-server パッケージ内（`packages/mcp-server/src/data/`）で管理するデータカテゴリと、ドメインモデルとの対応を以下に示す（MVP 時）。
...
```
外部 API ──→ scripts/fetch-base-data.ts ──→ 基本データ（JSON → src/data/）
                                                     │
チャンピオンズ差分 ──→ scripts/apply-champions-overrides.ts ──→ 最終データ（JSON → src/data/）
                                                                       │
                                               scripts/validate-data.ts ──→ バリデーション通過後にコミット
```
...
| 取得対象 | 出力先 | 備考 |
|---|---|---|
| ポケモン種族データ | `src/data/pokemon.json` | 種族値・タイプ・特性・覚える技 |
| 技データ | `src/data/moves.json` | 名前・タイプ・威力・命中率・分類 |
| 特性データ | `src/data/abilities.json` | 名前・効果説明 |
| タイプ相性 | `src/data/type-matchups.json` | 18×18 の倍率マトリクス |

性格データ（`src/data/natures.json`）はゲーム間で変わらない固定データのため、手動で作成しスクリプト対象外とする。

上記のパスはすべて `packages/mcp-server/` からの相対パスである。

#### Stage 2: チャンピオンズ差分適用（`apply-champions-overrides.ts`）

Stage 1 で取得した基本データに、チャンピオンズ固有の差分を上書き適用する。差分は `packages/mcp-server/scripts/overrides/` 配下の JSON ファイルに定義する。
...
データは mcp-server パッケージ（npm パッケージ名: `ai-rotom`）に同梱して npm publish する。データ更新時はパッケージのバージョンを上げて再 publish することで、ユーザーは `npx ai-rotom` で最新データを利用できる。

ゲームアップデートへの対応は、データファイルをバージョン管理（Git）でも追跡する。

#### ゲームアップデート時の更新手順

```
1. packages/mcp-server/scripts/fetch-base-data.ts を再実行し、基本データを最新化
2. packages/mcp-server/scripts/overrides/ の差分定義を更新（新たな変更点を反映）
3. packages/mcp-server/scripts/apply-champions-overrides.ts を実行し、差分を適用
4. packages/mcp-server/scripts/validate-data.ts を実行し、整合性を確認
5. Git diff で変更内容を確認し、コミット
6. mcp-server パッケージのバージョンを更新し、npm publish
```

#### データメタ情報

各データファイルの更新日時とゲームバージョンを追跡するため、`packages/mcp-server/src/data/meta.json` を配置する。
...
MCP ツール用の DTO。ドメイン層の DamageResult（VO）を拡張し、MCP 出力に必要な追加フィールド（moveName, moveType, attackerName, defenderName, typeEffectiveness, isStab, appliedConditions）を加えたものである。
...
| カバレッジ目標 | Domain 層: 100%、Application 層: 80% 以上、それ以外: 機能テスト中心 |
...
|---|---|---|
| `@ai-rotom/shared` | ドメインロジック・ユースケース | ユニットテスト中心 |
| `ai-rotom`（mcp-server） | ツール定義・入力バリデーション・名前解決・データの整合性 | 結合テスト + 静的検証 |

### 7.2 各レイヤーのテスト戦略

#### 7.2.1 shared パッケージ: Domain 層
...
**TypeMatchupEvaluator のテスト項目:**

| テスト対象 | テスト内容 |
|---|---|
| 単タイプ相性 | 18×18 の代表的な組み合わせが正しいか |
| 複合タイプ相性 | 2タイプの倍率乗算が正しいか（4倍・0.25倍・0倍を含む） |
| 弱点一覧 | getWeaknesses が正しい弱点タイプを返すか |
| 耐性一覧 | getResistances が正しい耐性タイプを返すか |
| 無効一覧 | getImmunities が正しい無効タイプを返すか |
...
| 先制技 | priority が高い技を持つ場合の行動順判定 |
| 優先度の競合 | 両者が先制技を持つ場合の判定 |

#### 7.2.2 shared パッケージ: Application 層

Repository をモックし、ユースケースのオーケストレーションが正しいことを検証する。

#### 7.2.3 mcp-server パッケージ
...
| 優先度 | 対象 | 理由 |
|---|---|---|
| **P0** | DamageCalculator（shared） | ダメージ計算の正確性がプロダクトの核心価値 |
| **P0** | StatCalculator（shared） | 実数値計算はダメージ計算の前提 |
| **P0** | TypeMatchupEvaluator（shared） | タイプ相性は全機能の基盤 |
| **P1** | mcp-server 内データの整合性 | データの欠落は計算結果の誤りに直結 |
| **P1** | ValueObject の生成制約（shared） | 不正なドメインオブジェクトの生成を防ぐ |
| **P2** | Application 層のユースケース（shared） | ドメインロジックが正しければ影響は限定的 |
| **P2** | mcp-server の名前解決・バリデーション | ユーザー体験に影響 |
| **P3** | Infrastructure 層の Repository 実装（mcp-server） | データ変換のロジックが単純 |
...
| # | 拡張項目 | 優先度 | 依存先 | 概要 |
|---|---|---|---|---|
| 1 | テラスタル対応 | 高 | shared パッケージ | ゲームアップデートで追加予定のテラスタル機能への対応 |
...
テラスタル対応: shared パッケージ + mcp-server 内データの拡張  ← 拡張 #1
```

API サーバー追加時の mcp-server の変更:
- Infrastructure 層の Repository 実装を JSON データ読み込みから HTTP クライアント（api-server 経由）に変更
- `src/data/` 配下の JSON データファイルを削除（データは api-server 側の DB で管理）
...
| shared の再利用 | Application 層の UseCase をそのまま呼び出す |
| Infrastructure 層 | DB からデータを取得する Repository 実装を持つ |
...
いずれの拡張も、shared パッケージのドメインロジックには手を加えず、新しいインターフェース層を追加する形で実現する（テラスタル対応を除く）。

### 8.6 npm publish 時の注意事項

npm publish 時に意図しないファイルが公開されないよう、package.json の `files` フィールドで公開ファイルを明示的に制御すること。特に `scripts/` ディレクトリや設定ファイル（tsconfig.json 等）が意図せず publish されないよう注意する。
