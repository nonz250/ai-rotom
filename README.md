...
1. npm で **Granular Access Token** を発行
   - Packages: `ai-rotom` のみ / Permissions: `Read and write`
   - 期限は短め（90日〜1年）を推奨
...
- **Settings → Environments → `production`** を作成
  ...
  - `NPM_TOKEN` はこの environment のシークレットに格納
...
初回 publish は `NPM_TOKEN` (Granular Access Token) を利用します。publish 実績ができた後、より安全な
[Trusted Publisher (OIDC)](https://docs.npmjs.com/trusted-publishers/)
への移行を推奨します（long-lived token の漏洩リスクを排除）。
